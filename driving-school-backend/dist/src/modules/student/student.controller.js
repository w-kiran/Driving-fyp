...
export const requestNewLesson = async (req, res) => {
    try {
        ...
        const { preferredSlot, preferredDate, vehicleType, trainingDuration, experienceLevel, lessonsCompleted = 0, failures = 0, examDate, } = req.body;
...
export const getMyBookings = async (_req, res) => {
    try {
...
export const getMyLessons = async (_req, res) => {
    try {
        const authHeader = _req.headers.authorization;
        if (!authHeader)
            return res.status(401).json({ message: "Unauthorized" });
